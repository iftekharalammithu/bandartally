...
export default function Worksheet() {
  ...
  const calculateTotalBags = (row) =>
    data[row].h1 + data[row].h2 + data[row].h3 + data[row].h4;
  const calculateTotalMTs = (bags) => (bags * 50) / 1000;
  const last24hrs =
  ...
  const handleInputChange = (row, hatch, value) => {
    setData((prev) => ({
      ...prev,
  ...
  const handleFocus = (row, hatch) => {
    setFocusedField(`${row}-${hatch}`);
  };

  const handleBlur = (row, hatch) => {
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Daily Discharging Report</h1>
      <Table className="border border-gray-400">
        <TableHeader>
          <TableRow className="border-b border-gray-400">
            <TableHead
              rowSpan={2}
              className="align-middle border-r border-gray-400 p-4"
            >
              As per
            </TableHead>
            ...
            <TableHead
              colSpan={2}
              className="text-left border-r border-gray-400"
            >
              Total
            </TableHead>
          </TableRow>
